  ...
  describe ".enum" do
    describe "positional args" do
      it "creates a new Enum based on the given keyword args" do
        subject = Nummy.enum(:FOO, :BAR, :BAZ)
    ...
    describe "keyword args" do
      it "creates a new Enum based on the given keyword args" do
        subject = Nummy.enum(FOO: 123, BAR: 456, BAZ: 789)
    ...
    describe "when a block is given" do
      it "calls the block" do
        block_called = false
        Nummy.enum(:FOO, :BAR, :BAZ) { block_called = true }
      ...
      it "yields the new enum to the block" do
        block_args = nil
        subject = Nummy.enum(:FOO, :BAR, :BAZ) { |*args| block_args = args }
      ...
      it "calls the block in the context of the new enum" do
        block_ctx = nil
        subject = Nummy.enum { block_ctx = self }
      ...
      it "calls the block after setting any constants" do
        block_pairs = nil
        Nummy.enum(FOO: 123, BAR: 456) { block_pairs = pairs }
    ...
    describe "mixed args" do
      it "creates a new Enum based on the given keyword args" do
        subject = Nummy.enum(:FOO, BAR: :kwarg) { self::BAZ = :block }
